...
- **课件预览的缩放 / 平移 / 逐屏**（`web/ios/js/app.js` 中的“课件预览视口”一节）
  ...
  - 画笔模式下单指绘制、双指缩放平移（第二指落下自动结束当前笔画），双击 2.5x，最大 8x，
    均与安卓一致；向前翻页落在上一页底部，便于向上逐屏回顾。
  ...
  - **点动翻页**（`bindCoursewareGestures` 里的 `paging`）：点画面左半边上一页、右半边下一页。
    ...
    与「双击放大」共用一个判定窗口：轻点后等 `CW_TAP_PAGE_DELAY_MS`(340ms) 再翻页，
    窗口内来了第二下就取消这次翻页、改走双击放大；移动超过 `CW_TAP_MOVE_SLOP_PX`(12px)
